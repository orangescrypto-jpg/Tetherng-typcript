...
const ROLES: { value: SignupRole; label: string; description: string; icon: React.ElementType; color: string }[] = [
  {
    value: 'tenant',
    label: 'I want to Buy/Rent',
    description: 'Looking for a property to buy or rent',
    icon: Home,
    color: 'border-brand-500 bg-brand-50 dark:bg-brand-500/10 text-brand-700 dark:text-brand-400',
  },
  {
    value: 'agent',
    label: "I'm an Agent",
    description: 'List properties and find buyers/tenants',
    icon: User,
    color: 'border-amber-500 bg-amber-50 dark:bg-gold-400/10 text-amber-700 dark:text-gold-400',
  },
  {
    value: 'landlord',
    label: "I'm a Landlord",
    description: 'Sell or rent out my own properties',
    icon: Building2,
    color: 'border-brand-400 bg-brand-50 dark:bg-brand-300/10 text-brand-600 dark:text-brand-300',
  },
];
    ...
    <div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div
            className={cn(
              ...
              step === 2 ? 'bg-brand-600' : 'bg-gray-200 dark:bg-dark-400',
            )}
          />
          <div
            className={cn(
              'flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold transition-all',
              step === 2 ? 'bg-brand-600 text-white' : 'bg-gray-200 dark:bg-dark-400 text-muted',
            )}
          >
            2
          </div>
      ...
      {step === 1 && (
        <div className="animate-fade-in">
          ...
          {errors.role && (
            <p className="mt-3 text-xs text-red-600 dark:text-danger">{errors.role.message}</p>
          )}
          ...
          {selectedRole === 'agent' && (
            <div className="mt-6 rounded-xl bg-amber-50 dark:bg-gold-400/5 border border-amber-200 dark:border-gold-400/20 p-4 animate-slide-up">
              ...
              <p className="mt-1 text-xs leading-relaxed text-muted">
                Verified agents get 3x more inquiries. You can verify right after signing up for
                just ₦2,500.
              </p>
            </div>
          )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="animate-fade-in space-y-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-muted capitalize">Signing up as {selectedRole}</p>
            </div>
            <button
              type="button"
              onClick={() => setStep(1)}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className={cn(
                  'input-field',
                  errors.firstName && 'border-red-500 dark:border-danger',
                )}
                {...register('firstName')}
              />
              {errors.firstName && (
                <p className="mt-1 text-xs text-red-600 dark:text-danger">
            ...
            <div>
              ...
              <input
                ...
                className={cn(
                  'input-field',
                  errors.lastName && 'border-red-500 dark:border-danger',
                )}
                {...register('lastName')}
              />
              {errors.lastName && (
          ...
          <div>
            ...
            <input
              ...
              className={cn(
                'input-field',
                errors.email && 'border-red-500 dark:border-danger',
              )}
              {...register('email')}
            />
            {errors.email && (
              <p className="mt-1 text-xs text-red-600 dark:text-danger">{errors.email.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            <div className="flex">
              <span className="inline-flex items-center rounded-l-xl border border-r-0 border-gray-300 dark:border-dark-400 bg-gray-100 dark:bg-dark-200 px-3 text-sm text-muted">
                +234
              </span>
              <input
                placeholder="801 234 5678"
                className={cn(
                  'input-field rounded-l-none',
                  errors.phone && 'border-red-500 dark:border-danger',
                )}
                {...register('phone')}
              />
            </div>
            {errors.phone && (
              <p className="mt-1 text-xs text-red-600 dark:text-danger">{errors.phone.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className={cn(
                  'input-field pr-11',
                  errors.password && 'border-red-500 dark:border-danger',
                )}
                {...register('password')}
              />
              <button
                ...
                className="absolute top-1/2 right-3 -translate-y-1/2 p-1 text-muted hover:text-gray-900 dark:hover:text-white transition-colors"
              >
          ...
          <div>
            ...
            <input
              ...
              className={cn(
                'input-field',
                errors.confirmPassword && 'border-red-500 dark:border-danger',
              )}
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            <input
              type="checkbox"
              className="mt-0.5 h-4 w-4 rounded border-gray-300 dark:border-dark-400 bg-white dark:bg-dark-50 text-brand-600 focus:ring-brand-500/50"
              {...register('agreeTerms')}
            />
          ...
          {errors.agreeTerms && (
            <p className="-mt-3 text-xs text-red-600 dark:text-danger">
