...
const FEATURED_LISTINGS = [
  { id: '1', title: 'Modern 3-Bedroom Duplex in Lekki Phase 1', location: 'Lekki, Lagos', price: 8500000, priceLabel: 'per year', image: 'https://picsum.photos/seed/tether-lekki/600/400.jpg', beds: 3, baths: 4, boostType: 'featured' as const, isPropertyVerified: true, agentVerified: true, viewsCount: 1247 },
  { id: '2', title: 'Luxury 2-Bed Apartment — Banana Island', location: 'Ikoyi, Lagos', price: 12000000, priceLabel: 'per year', image: 'https://picsum.photos/seed/tether-ikoyi/600/400.jpg', beds: 2, baths: 2, boostType: 'top_placement' as const, isPropertyVerified: true, agentVerified: true, viewsCount: 2103 },
  { id: '3', title: 'Spacious 4-Bed Terrace with Boys Quarter', location: 'Gwarinpa, Abuja', price: 45000000, priceLabel: 'for sale', image: 'https://picsum.photos/seed/tether-abuja/600/400.jpg', beds: 4, baths: 5, boostType: 'urgent' as const, isPropertyVerified: false, agentVerified: true, viewsCount: 876 },
];
...
const TRUST_FEATURES = [
  { icon: Shield, title: 'Escrow Protection', description: 'Your payment is held securely until you confirm everything is right. No more lost funds.' },
  { icon: CheckCircle2, title: 'BVN Verified Agents', description: 'Every agent on TetherNG has passed identity verification. Know exactly who you\'re dealing with.' },
  { icon: Building2, title: 'Property Verification', description: 'We physically verify properties before they get the green badge. What you see is what you get.' },
  { icon: Eye, title: 'Transparent Process', description: 'Track every step of your transaction in real-time. From deposit to completion, nothing is hidden.' },
];
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-dark">
      {/* Nav */}
      <header className="glass fixed top-0 left-0 right-0 z-50 border-b border-gray-200/50 dark:border-dark-400/50">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
          ...
          <nav className="hidden items-center gap-6 md:flex">
            <a href="#listings" className="text-sm font-medium text-gray-600 dark:text-muted hover:text-gray-900 dark:hover:text-white transition-colors">Listings</a>
            <a href="#how-it-works" className="text-sm font-medium text-gray-600 dark:text-muted hover:text-gray-900 dark:hover:text-white transition-colors">How It Works</a>
            <a href="#pricing" className="text-sm font-medium text-gray-600 dark:text-muted hover:text-gray-900 dark:hover:text-white transition-colors">Pricing</a>
            <ThemeToggle />
          ...
          <div className="flex items-center gap-2 md:hidden">
            <ThemeToggle />
            <button className="p-2 text-gray-600 dark:text-muted hover:text-gray-900 dark:hover:text-white" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
              {mobileMenuOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
            </button>
          </div>
        </div>
        {mobileMenuOpen && (
          <div className="animate-slide-up border-t border-gray-200/50 dark:border-dark-400/50 bg-white dark:bg-dark-50/95 backdrop-blur-xl md:hidden">
            <div className="flex flex-col gap-1 px-4 py-4">
              <a href="#listings" className="rounded-lg px-3 py-2.5 text-sm font-medium text-gray-600 dark:text-muted hover:bg-gray-100 dark:hover:bg-dark-200 hover:text-gray-900 dark:hover:text-white" onClick={() => setMobileMenuOpen(false)}>Listings</a>
              <a href="#how-it-works" className="rounded-lg px-3 py-2.5 text-sm font-medium text-gray-600 dark:text-muted hover:bg-gray-100 dark:hover:bg-dark-200 hover:text-gray-900 dark:hover:text-white" onClick={() => setMobileMenuOpen(false)}>How It Works</a>
              <a href="#pricing" className="rounded-lg px-3 py-2.5 text-sm font-medium text-gray-600 dark:text-muted hover:bg-gray-100 dark:hover:bg-dark-200 hover:text-gray-900 dark:hover:text-white" onClick={() => setMobileMenuOpen(false)}>Pricing</a>
              <div className="mt-2 flex flex-col gap-2 border-t border-gray-200 dark:border-dark-400/50 pt-3">
      ...
      <section className="relative overflow-hidden pt-28 pb-20 sm:pt-36 sm:pb-28">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mb-8 flex flex-wrap items-center justify-center gap-3">
            ...
            <span className="badge-featured"><Star className="h-3 w-3" /> Premium Listings</span>
          </div>
      ...
      {/* Stats */}
      <section className="border-y border-gray-200 dark:border-dark-400 bg-white dark:bg-dark-50/50">
      ...
      <section id="listings" className="py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {FEATURED_LISTINGS.map((listing) => (<ListingCard key={listing.id} listing={listing} />))}
          </div>
          <div className="mt-10 text-center">
            <Link to="/listings" className="btn-outline inline-flex items-center gap-2 px-8 py-3">View All Properties <ArrowRight className="h-4 w-4" /></Link>
          </div>
        </div>
      </section>

      {/* How It Works */}
      <section id="how-it-works" className="border-y border-gray-200 dark:border-dark-400 bg-white dark:bg-dark-50/30 py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-14 grid gap-8 md:grid-cols-2 lg:grid-cols-4">
            {[
              { step: '01', title: 'Find Your Property', desc: 'Browse verified listings from trusted agents across Nigeria.', color: 'text-brand-600 dark:text-brand-400' },
              { step: '02', title: 'Start Escrow', desc: 'Initiate a secure transaction — your money goes into escrow, not to the agent directly.', color: 'text-amber-600 dark:text-gold-400' },
              { step: '03', title: 'Inspect & Confirm', desc: 'Visit the property, verify it matches the listing, and confirm satisfaction.', color: 'text-brand-500 dark:text-brand-300' },
              { step: '04', title: 'Funds Released', desc: 'Once you confirm, payment is released to the agent or seller. Everyone wins.', color: 'text-green-600 dark:text-success' },
            ].map((item) => (
      ...
      {/* Pricing */}
      <section id="pricing" className="border-y border-gray-200 dark:border-dark-400 bg-white dark:bg-dark-50/30 py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center">
          ...
          <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <div key={plan.name} className={cn('rounded-2xl border p-6 flex flex-col transition-all duration-300', plan.featured ? 'pricing-popular scale-[1.02]' : 'border-gray-200 dark:border-dark-400 bg-white dark:bg-dark-100 hover:border-gray-300 dark:hover:border-dark-500')}>
                {plan.featured && <span className="badge-featured self-start mb-3"><Star className="h-3 w-3" /> Most Popular</span>}
                <div>
      ...
      {/* Footer */}
      <footer className="border-t border-gray-200 dark:border-dark-400 bg-white dark:bg-dark-50/30 py-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
  ...
  return (
    <div className={cn(config.wrapper, 'group cursor-pointer')}>
      <div className="relative aspect-[16/10] overflow-hidden rounded-xl">
        <img src={listing.image} alt={listing.title} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
        <div className="absolute top-3 left-3 flex flex-wrap gap-2">
          {config.badge?.type === 'featured' && <span className="badge-featured"><Star className="h-3 w-3" /> Featured</span>}
          {config.badge?.type === 'pinned' && <span className="inline-flex items-center gap-1 rounded-lg bg-brand-600/90 px-2.5 py-1 text-xs font-bold text-white"><Rocket className="h-3 w-3" /> Top of Search</span>}
          {config.badge?.type === 'urgent' && <span className="badge-urgent"><Flame className="h-3 w-3" /> Urgent</span>}
        ...
        <div className="absolute bottom-3 left-3">
          <span className="rounded-lg bg-black/60 backdrop-blur-sm px-3 py-1.5 text-sm font-bold text-white">
            {formatCurrencyShort(listing.price)}<span className="ml-1 text-xs font-normal text-white/70">{listing.priceLabel}</span>
          </span>
        </div>
      </div>
      <div className="mt-4 px-1 pb-1">
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          {hasBoost && <span className="text-[10px] font-medium text-amber-600 dark:text-gold-400">{listing.viewsCount.toLocaleString()} views</span>}
        </div>
